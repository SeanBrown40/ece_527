...
class Warden(object):
    ...
    def __init__(self, obs_r, pos, spd, collect_r, max_buffer_size, movable=True, mv_bt=0, trans_bt=0):  # pos(x,y,h)
        self.no = Warden.warden_count
        Warden.warden_count += 1
...
class MEC_world(object):
    def __init__(self, map_size, agent_num, sensor_num, warden_num, obs_r, speed, collect_r, max_size=1, sensor_lam=0.5):
        self.agents = []
        self.sensors = []
        self.wardens = []
        ...
        self.warden_pos_init = [random.sample([i for i in range(int(0.15 * self.map_size), int(0.85 * self.map_size))], warden_num),
                               random.sample([i for i in range(int(0.15 * self.map_size), int(0.85 * self.map_size))], warden_num)]


        for i in range(warden_num):
            self.wardens.append(
                Warden(self.obs_r, np.array([self.warden_pos_init[0][i], self.warden_pos_init[1][i]]), speed, collect_r, self.max_buffer_size))
