...
class MEC_MARL_ENV(gym.Env):
    ...
    def __init__(self, world, alpha=0.5, beta=0.2, aggregate_reward=False, discrete=True,
        ...
        self.wardens = self.world.wardens
        self.warden_num = self.world.warden_count
    ...
    def render(self, name=None, epoch=None, save=False):
        ...
        for warden in self.world.wardens:
            plt.scatter(warden.position[0], warden.position[1], c='green', alpha=0.9)
            plt.annotate(warden.no + 1, xy=(warden.position[0], warden.position[1]), xytext=(warden.position[0] + 0.1, warden.position[1] + 0.1))
            obs_plot = get_circle_plot(warden.position, self.obs_r/2)
            collect_plot = get_circle_plot(warden.position, self.collect_r)
            plt.fill_between(obs_plot[0], obs_plot[1], obs_plot[2], where=obs_plot[1] > obs_plot[2], color='green', alpha=0.02)
            plt.fill_between(collect_plot[0], collect_plot[1], collect_plot[2], where=collect_plot[1] > collect_plot[2], color='green', alpha=0.05)

        plt.grid()
        plt.xlabel('x')
        plt.ylabel('y')
        plt.legend(['Sensors', 'Edge Agents', 'Wardens'])
        plt.axis('square')
